...
import { Document, Page, pdf, StyleSheet, Font, Text, View, Image as PdfImage } from '@react-pdf/renderer';
...
`);
  ...
  const handleDownloadPdf = async () => {
    ...
    // Pre-fetch all images in the HTML as base64 data URLs using a canvas.
    // This is needed because the react-pdf worker cannot fetch external images
    // from a browser context due to CORS restrictions.
    const fetchImageAsBase64 = (src) => new Promise((resolve) => {
      ...
      img.onerror = () => resolve(null); // Skip images that fail to load
      img.src = src;
    });
    ...
    try {
      ...
      // Convert all image src attributes in the HTML to base64 so the PDF renderer can embed them
      let processableHtml = pdfReadyHtml;
      const imgSrcMatches = [...pdfReadyHtml.matchAll(/<img[^>]+src="([^"]+)"/g)];
      for (const match of imgSrcMatches) {
        const originalSrc = match[1];
        if (!originalSrc.startsWith('data:')) {
          const base64 = await fetchImageAsBase64(originalSrc);
          if (base64) {
            processableHtml = processableHtml.replace(originalSrc, base64);
          }
        }
      }
          ...
          <Page size="A4" style={pdfStyles.page}>
            <Html 
              ...
              components={{
                ...
                // del is generated by marked for ~~strikethrough~~ but react-pdf-html drops it silently.
                // We override it to render as Text with line-through decoration.
                del: ({ style, children }) => (
                  <Text style={[style, { textDecoration: 'line-through' }]}>{children}</Text>
                ),
                s: ({ style, children }) => (
                  <Text style={[style, { textDecoration: 'line-through' }]}>{children}</Text>
                ),
